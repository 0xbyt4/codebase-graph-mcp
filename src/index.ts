...
const WRITES_PAGES_TOOL = { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false } as const;
...
server.registerTool(
  "visualize_graph",
  {
    ...
    description:
      "Generate an interactive HTML dependency graph and try to open it in the browser. Files are nodes colored by directory, edges are dependency arrows, node size reflects how many files import it. With atlas=true it writes a whole folder instead: one graph for the project, one per major directory, and an index page with totals, hotspots and cycles.",
    inputSchema: {
      top: z
        .number()
        .int()
    ...
    try {
      ...
      if (atlas) {
        ...
        const pages: [string, string][] = [
          ...
          ...entries.map((e) => [e.href, e.page.html] as [string, string]),
        ];

        // Check every target before writing any, so a refusal leaves nothing half done
        for (const [name] of pages) await assertOverwritable(join(outDir, name));
        await mkdir(outDir, { recursive: true });
        for (const [name, html] of pages) await writeFile(join(outDir, name), html, "utf-8");
...
async function assertOverwritable(path: string): Promise<void> {
  ...
  if (stat.isSymbolicLink()) throw new Error(`refusing to write ${path}: it is a symbolic link`);
  if (!stat.isFile()) throw new Error(`refusing to write ${path}: not a regular file`);
