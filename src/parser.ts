...
const TS_LINE_PATTERNS: { pattern: RegExp; typeGroup?: number; specGroup: number; statement?: boolean }[] = [
...
export interface ParsedImport {
  ...
  // True for imports erased at runtime: TS `import type` / `export type`
  // and Python imports under `if TYPE_CHECKING:`.
  typeOnly: boolean;
  // True for Rust `mod name;`: the parent file declares (contains) the child
  // module. A child referring back to its parent via `crate::`/`super::` is
  // normal in Rust, so these edges never count towards a cycle.
  declaration?: boolean;
}
...
export function parseImports(
  ...
  if (content.length > MAX_PARSE_BYTES) return [];
...
class ImportCollector {
  private byKey = new Map<string, ParsedImport>();

  add(raw: string, resolved: string | null, typeOnly: boolean, declaration = false): void {
    const existing = this.byKey.get(raw);
    if (existing) {
      if (existing.typeOnly && !typeOnly) existing.typeOnly = false;
      return;
    }
    const entry: ParsedImport = { raw, resolved, typeOnly };
    if (declaration) entry.declaration = true;
...
function startsStatement(line: string, index: number): boolean {
...
function parseTypeScript(
  ...
  for (const line of lines) {
    ...
    if (isCommentLine(trimmed) || trimmed.length > MAX_LINE_LENGTH) continue;

    for (const { pattern, typeGroup, specGroup, statement } of TS_LINE_PATTERNS) {
      for (const match of trimmed.matchAll(pattern)) {
        if (statement && !startsStatement(trimmed, match.index ?? 0)) continue;
...
function collapseMultiLineImports(content: string): string {
  return content.replace(
    /^([ \t]*(?:import|export)\s+(?:type\s+)?(?:[\w$]+\s*,\s*)?)\{([^}]*)\}(\s*from\s*['"][^'"\n]+['"])/gm,
...
function resolveTypeScriptFile(base: string): string | null {
  ...
  for (const tryExt of TS_JS_EXTENSIONS) {
    if (isFile(base + tryExt)) return base + tryExt;
  }
...
interface AliasConfig {
  // Directory `paths` targets are relative to (baseUrl, or the config's dir).
  baseDir: string;
  baseUrl: string | null;
...
function findAliasConfig(dir: string, projectRoot?: string): AliasConfig | null {
  ...
  if (!result) {
    ...
    if (parent !== dir && !atRoot) result = findAliasConfig(parent, projectRoot);
  }
...
function loadAliasConfig(configPath: string, seen: Set<string>): AliasConfig | null {
  ...
  const config = parsed as { extends?: unknown; compilerOptions?: unknown };

  const configDir = dirname(configPath);
  // tsc merges `extends` entries per compiler option, later entries winning.
  const base: AliasConfig = { baseDir: configDir, baseUrl: null, paths: {} };
  let inherited = false;
  ...
  for (const parent of parents) {
    ...
    if (typeof parent !== "string" || (!parent.startsWith(".") && !isAbsolute(parent))) continue;
    let parentPath = resolve(configDir, parent);
    if (!isFile(parentPath) && isFile(parentPath + ".json")) parentPath += ".json";
    const loaded = loadAliasConfig(parentPath, seen);
    if (!loaded) continue;
    inherited = true;
    if (loaded.baseUrl !== null) base.baseUrl = loaded.baseUrl;
  ...
  const options = (config.compilerOptions && typeof config.compilerOptions === "object" ? config.compilerOptions : {}) as {
    baseUrl?: unknown;
    paths?: unknown;
  };
  const ownBaseUrl = typeof options.baseUrl === "string" ? resolve(configDir, options.baseUrl) : null;
  const ownPaths = options.paths && typeof options.paths === "object" ? sanitizePaths(options.paths as Record<string, unknown>) : null;
...
function sanitizePaths(raw: Record<string, unknown>): Record<string, string[]> {
  const out: Record<string, string[]> = {};
  for (const [pattern, targets] of Object.entries(raw)) {
    if (Array.isArray(targets)) out[pattern] = targets.filter((t): t is string => typeof t === "string");
  }
  return out;
}
...
function aliasCandidates(importPath: string, config: AliasConfig): string[] {
  const candidates: string[] = [];

  const matches: { prefixLength: number; targets: string[]; star: string }[] = [];
  for (const [pattern, targets] of Object.entries(config.paths)) {
    const starIndex = pattern.indexOf("*");
    if (starIndex < 0) {
      if (pattern === importPath) matches.push({ prefixLength: pattern.length + 1, targets, star: "" });
      continue;
    }
  ...
  if (config.baseUrl) candidates.push(resolve(config.baseUrl, importPath));
  return candidates;
}
...
function parsePython(
  ...
  for (const line of lines) {
    ...
    if (inTripleQuote || !trimmed || trimmed.startsWith("#")) continue;

    const indent = line.length - line.trimStart().length;
    if (typeCheckingIndent >= 0 && indent <= typeCheckingIndent) typeCheckingIndent = -1;
...
function collapsePythonParenImports(content: string): string {
  return content.replace(
    /^([ \t]*from\s+\S+\s+import\s*)\(([^)]*)\)/gm,
    (_match, prefix: string, names: string) =>
      `${prefix}${names.replace(/#[^\n]*/g, "").replace(/\s+/g, " ").trim()}`,
  );
}

// "a, b as c, d" -> ["a", "b", "d"]; strips a trailing comment.
function parsePythonNames(list: string): string[] {
  return list
    .split("#")[0]
    .split(",")
...
function resolvePythonFromImport(
  modulePath: string,
  names: string[],
  fromFile: string,
  projectRoot?: string,
  ...
  const results: { raw: string; resolved: string | null }[] = [];
  ...
  if (packageDir) {
    ...
    for (const name of names) {
      ...
      if (sub) results.push({ raw: `${prefix}${name}`, resolved: sub });
      else attributeNames = true; // defined in the package's __init__.py
    }
  } else {
    attributeNames = true;
  }
  if (attributeNames || results.length === 0) results.push({ raw: modulePath || ".", resolved: moduleFile });
  return results;
}
...
function pythonSearchRoots(fromFile: string, projectRoot?: string): string[] {
...
function pythonRelativeBase(dots: number, fromFile: string): string {
...
function resolvePythonModulePath(modulePath: string, baseDir: string): string | null {
...
function parseRust(
  ...
  // enclose a `use`.
  let depth = 0;
  const inlineModDepths: number[] = [];

  for (const line of lines) {
    const trimmed = line.trim();
    if (isCommentLine(trimmed) || trimmed.length > MAX_LINE_LENGTH) continue;

    if (RUST_INLINE_MOD_PATTERN.test(trimmed)) inlineModDepths.push(depth + 1);
    ...
    while (inlineModDepths.length && depth < inlineModDepths[inlineModDepths.length - 1]) inlineModDepths.pop();
  }
...
function expandRustUseTree(tree: string): string[] {
  ...
  for (const item of splitTopLevel(inner)) {
    if (!item) continue;
    if (item === "self") results.push(prefix);
    else results.push(...expandRustUseTree(item).map((p) => (prefix ? `${prefix}::${p}` : p)));
  }
  return results;
}
...
function resolveRustUsePath(
  ...
  if (parts[0] === "crate") {
    ...
    if (parts[0] !== "super") return fromFile;
    dir = getRustModuleDir(fromFile);
    while (parts[0] === "super") {
      dir = dirname(dir);
      parts.shift();
    ...
    if (inlineModLevels > 0 || parts.length === 1) return null;
    dir = getRustModuleDir(fromFile);
...
function rustModuleFileForDir(dir: string): string | null {
...
function findCrateSrcDir(fromFile: string, projectRoot?: string): string | null {
  let dir = dirname(fromFile);
  while (true) {
    if (isFile(join(dir, "Cargo.toml")) && existsSync(join(dir, "src"))) return join(dir, "src");
    const parent = dirname(dir);
    if (parent === dir || (projectRoot && !parent.startsWith(projectRoot))) break;
    dir = parent;
  }
...
function resolveRustModDeclaration(name: string, fromFile: string): string | null {
...
function resolveRustModulePath(parts: string[], baseDir: string): string | null {
