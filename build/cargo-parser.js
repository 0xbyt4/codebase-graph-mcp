 ...
 */
export function parseCargoDependencies(projectRoot) {
 ...
 */
function parseWorkspaceDeps(content, projectRoot) {
    const deps = new Map();
    ...
    for (const line of lines) {
        ...
        const pathMatch = trimmed.match(/^([\w-]+)\s*=\s*\{[^}]*path\s*=\s*"([^"]+)"[^}]*\}/);
        if (!pathMatch)
            continue;
 ...
 */
function parseWorkspaceMembers(content, projectRoot) {
 ...
 */
function extractCrateWorkspaceDeps(content) {
    const deps = [];
    const lines = content.split("\n");
    let depType = "normal";
    for (const line of lines) {
        ...
        // Match: package = { workspace = true, ... }
        const inlineMatch = trimmed.match(/^([\w-]+)\s*=\s*\{[^}]*workspace\s*=\s*true[^}]*\}/);
        if (inlineMatch) {
...
function getCrateEntryPoint(crateDir) {
