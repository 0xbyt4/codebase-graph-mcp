 ...
 */
export function parseConfigDependencies(projectRoot) {
    ...
    for (const entry of entries) {
        if (!entry.endsWith(".json5") && !entry.endsWith(".json"))
            continue;
...
function resolveConfigObject(obj, configFile, projectRoot) {
    ...
    if (root.cortex_llm && typeof root.cortex_llm === "object") {
        const llm = root.cortex_llm;
        if (typeof llm.type === "string") {
            const resolved = resolveClassInPluginDir(llm.type, "src/llm/plugins", projectRoot);
            if (resolved) {
                results.push({
                    configFile,
                    targetFile: resolved,
    ...
    if (Array.isArray(root.agent_inputs)) {
        for (const input of root.agent_inputs) {
            if (input && typeof input === "object" && typeof input.type === "string") {
                const resolved = resolveClassInPluginDir(input.type, "src/inputs/plugins", projectRoot);
                if (resolved) {
                    results.push({
                        configFile,
                        targetFile: resolved,
    ...
    if (Array.isArray(root.agent_actions)) {
        for (const action of root.agent_actions) {
            ...
            if (typeof act.name === "string") {
                const deps = resolveActionConfig(act.name, typeof act.connector === "string" ? act.connector : undefined, projectRoot);
    ...
    if (Array.isArray(root.simulators)) {
        for (const sim of root.simulators) {
            if (sim && typeof sim === "object" && typeof sim.type === "string") {
                const resolved = resolveClassInPluginDir(sim.type, "src/simulators/plugins", projectRoot);
                if (resolved) {
                    results.push({
                        configFile,
                        targetFile: resolved,
    ...
    if (Array.isArray(root.backgrounds)) {
        for (const bg of root.backgrounds) {
            if (bg && typeof bg === "object" && typeof bg.type === "string") {
                const resolved = resolveClassInPluginDir(bg.type, "src/backgrounds/plugins", projectRoot);
                if (resolved) {
                    results.push({
                        configFile,
                        targetFile: resolved,
 ...
 */
export function resolveClassInPluginDir(className, pluginsDir, projectRoot) {
 ...
 */
export function resolveActionConfig(name, connector, projectRoot) {
    const results = [];
    const interfacePath = join(projectRoot, "src", "actions", name, "interface.py");
    ...
    if (connector) {
        const connectorPath = join(projectRoot, "src", "actions", name, "connector", `${connector}.py`);
 ...
 */
export function resolveHookConfig(moduleName, projectRoot) {
